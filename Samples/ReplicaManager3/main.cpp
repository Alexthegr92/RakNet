...
struct SampleReplica : public Replica3
{
	...
	void RandomizeVariables(void)
	{
		if (randomMT() % 2)
		{
			var1Unreliable = RandomFloat(-10.0f, 10.0f);
			printf("var1Unreliable changed to %f\n", var1Unreliable);
		}
		if (randomMT() % 2)
		{
			var2Unreliable = RandomFloat(-10.0f, 10.0f);
			printf("var2Unreliable changed to %f\n", var2Unreliable);
		}
		if (randomMT() % 2)
		{
			var3Reliable = RandomFloat(-10.0f, 10.0f);
			printf("var3Reliable changed to %f\n", var3Reliable);
		}
		if (randomMT() % 2)
		{
			var4Reliable = RandomFloat(-10.0f, 10.0f);
			printf("var4Reliable changed to %f\n", var4Reliable);
		}
	}
